...
def _apply_hint(body, text):
    ...
    if m:
        ...
        if k in b:
            b.pop(k, None)
            # ═══ РЕМОНТ, УБРАВШИЙ СУЖАЮЩЕЕ ПОЛЕ, МЕНЯЕТ ВОПРОС - И ГОВОРИТ ОБ ЭТОМ ═══
            # ЖИВОЙ ПРОГОН ВЛАДЕЛЬЦА 26.09: ручка ответила «Field 'token_address' is not
            # recognized», ремонт поле убрал, запрос прошёл - и вернул топ-100 holdings умных
            # денег ПО ВСЕЙ СЕТИ там, где спрашивали ОДИН токен. Формально успех: 200, данные
            # настоящие. Фактически ответ на другой вопрос, и никто об этом не сказал.
            # Убрать поле всё равно ПРАВИЛЬНО (иначе запрос не пройдёт вовсе), но вызывающий
            # обязан узнать, что его вопрос сузился, - поэтому предупреждение уезжает и в лог,
            # и в текст правки, который читает человек.
            _why = 'убрал поле %r (площадка его не знает)' % k
            if k in _NARROWING:
                print('[nansen] ВНИМАНИЕ: убрано СУЖАЮЩЕЕ поле %r - ответ будет ШИРЕ запроса, '
                      'отбирать нужное придётся вызывающему' % k)
                _why += '; ВНИМАНИЕ: оно сужало запрос - ответ придёт шире'
            return b, _why
    ...
    if m:
        ...
        # ПОРЯДОК ЭТИХ ДВУХ ОТКАЗОВ ВАЖЕН, И ЕГО ВЫБРАЛ ТЕСТ. Если площадка просит СПИСОК, а
        # пары в `_SINGULAR_OF` нет - полезнее сказать «добавь пару», потому что построить поле
        # мы УМЕЕМ, как только получим одиночное значение. Сообщение про сужение здесь было бы
        # верным, но бесполезным: оно не говорит, что делать.
        ...
        # ═══ ИЗМЕРЕННЫЙ ДЕФОЛТ ПРОБУЕМ РАНЬШЕ ЛЮБЫХ ЗАПРЕТОВ ═══
        # ПЕРВАЯ РЕДАКЦИЯ ПОСТАВИЛА ПРОВЕРКУ `_NARROWING` ВЫШЕ ЭТОЙ СТРОКИ И СЛОМАЛА РАБОТАВШИЙ
        # РЕМОНТ: набор конкурса покраснел четырьмя проверками сразу («обязательное поле
        # подставлено», «кругов ровно три», «человек видит позиции»). Причина в порядке, а не в
        # идее: `_FIX_DEFAULTS` - это дефолты, СНЯТЫЕ живыми вызовами и годами работающие, и
        # запрещать их ради нового правила значит лечить здоровое.
        mk = _FIX_DEFAULTS.get(k)
        if mk is not None:
            b[k] = mk()
            _why = 'подставил обязательное поле %r' % k
            if k in _NARROWING:
                # ДЕФОЛТ ЕСТЬ, НО ПОЛЕ СУЖАЮЩЕЕ - ПОДСТАВЛЯЕМ И ГОВОРИМ ВСЛУХ. «Окно за 7 дней»
                # вместо спрошенного это единственный способ пройти запрос, но вызывающий обязан
                # знать, что период теперь НАШ, а не его.
                _why += '; ВНИМАНИЕ: поле сужает запрос - период/область теперь наши, не ваши'
            return b, _why
        if k in _NARROWING:
            # ДЕФОЛТА НЕТ, И ПОЛЕ СУЖАЮЩЕЕ - ОТКАЗ ТОЧНЕЕ, ЧЕМ «НЕЧЕМ»: он говорит, ПОЧЕМУ мы
            # не станем его выдумывать, а не только что не можем.
            return None, ('нужно поле %r, и оно СУЖАЕТ запрос - выдумать его значило бы ответить '
                          'не на тот вопрос' % k)
...
#: ПОЛЯ, КОТОРЫЕ СУЖАЮТ ЗАПРОС. Убрать такое поле - значит спросить ШИРЕ, чем просили;
#: подставить дефолтом - значит спросить ДРУГОЕ. И то и другое даёт настоящие данные не на наш
#: вопрос, то есть самую тихую разновидность лжи: ответ есть, он верен и он не о том.
#: Список закрытый и пополняется руками: «похоже на фильтр» здесь не годится.
_NARROWING = {'token_address', 'token_addresses', 'address', 'addresses', 'wallet',
              'symbol', 'symbols', 'market_id', 'chain', 'chains', 'date', 'date_range'}
