...
| `docs/CATALOG.md` | **the full catalogue: all 24 screens, what to type, which button, which endpoints, what it costs, and a one-line hook for each** — generated by a tool that refuses to write it if a command, an endpoint, a scene or a menu key does not actually exist |
| `docs/scenarios.md` | the longer walkthrough of each scenario |
...
`docs/CATALOG.md` lists every screen in one place: what to type, which button, which endpoints it
touches, what it costs, why it exists, and a one-line hook.

It is **generated**, and the generator refuses to write it when any technical claim fails: the
command is checked against the bot's own router, the endpoint against the client, the telemetry
scene against the closed registry, the menu key against the label registry. The reason is
narrow: that document is what public claims get written from, and promising a feature that does
not exist is worse than promising nothing.

Prices in it are given in credits where Nansen's published list names one, and in **number of
requests** where it does not. That is not hedging — an invented credit number in the document
people quote from would be a lie in the most verifiable place.
