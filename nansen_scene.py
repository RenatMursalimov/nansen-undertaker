...
def pm_markets_data(lang='ru', top=12, rows=None):
    ...
    чтобы экран повесил его на тап, - ровно как кнопка рынка под списком в чате.
    """
    N = _n()
    if rows is None:
...
# ВНЕ себя. Здесь номер приезжает из уже открытой карточки рынка, а в чате - кнопкой на карточке
# рынка, куда ведёт тап по самому рынку в списке. Тот же дефект и то же лечение, что было с
# графиком и стаканом.
# ═══════════════════════════════════════════════════════════════════════════════
