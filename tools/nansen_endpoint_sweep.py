...
def registry():
    ...
    base = [
        ...
        # position-detail: ТОЛЬКО market_id (круг 3 пробы: поля адреса ручка не знает вовсе).
        _case('prediction-market/position-detail',
              {'market_id': MARKET, 'pagination': _pg()}),
        # jup-dca: нативные токены не поддерживаются ПО УСТРОЙСТВУ, поэтому обычный mint.
        _case('tgm/jup-dca',
