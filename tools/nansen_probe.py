...
def _cases_next():
    """ВОСЕМЬ РУЧЕК, НА КОТОРЫХ СТОЯТ СЛЕДУЮЩИЕ ФИЧИ, И НИ У ОДНОЙ СХЕМА НЕ СНЯТА.

    ЗАЧЕМ ЭТА ГРУППА ПОЯВИЛАСЬ ОТДЕЛЬНО. Список идей упирается ровно в это: пока неизвестно
    имя обязательного поля, фича не пишется - она УГАДЫВАЕТСЯ, и на первом тапе отдаёт 422.
    Это уже случалось дважды за неделю (`tgm/perp-positions`: посылали `token`, надо
    `token_symbol`; `ra-agent/posts-by-token`: `token_address` не знают вовсе, знают
    `token_symbol`). Поэтому порядок такой: сперва проба, потом код, а не наоборот.

    РЕМОНТ ПО СЛОВАМ ПЛОЩАДКИ ЗДЕСЬ ВЫКЛЮЧЕН НАРОЧНО (группы нет в `FIX_DEFAULT`). Ремонт
    снимает поля, которых площадка не знает, - и на поиске ИМЕНИ поля он выкидывает сам вопрос,
    возвращая `200 data:[]`. Формально успех, по существу «дай постов без фильтра». Нам нужен
    сырой отказ: он это имя и называет.

    ЧТО ДЕЛАТЬ С ВЫВОДОМ. Для каждой ручки ответ - один из трёх: (1) 200 + имена полей первой
    строки -> фичу можно писать, поля известны; (2) 422 с текстом про поле -> дослать поле и
    прогнать снова; (3) 404 -> ручки нет по этому пути, и тогда её похороны честны.
    """
    _pg = {"pagination": {"page": 1, "per_page": 5}}
    return [
        # ── №9 трекер DCA: «кто набирает равными кусками» ────────────────────────────────
        ('smart-money/dcas', 'DCA умных денег: тело по образцу соседей smart-money',
         dict(_pg, chains=["ethereum", "base"])),
        ('smart-money/dcas', 'вариант без chains - вдруг он обязателен наоборот', dict(_pg)),
        ('tgm/jup-dca', 'DCA на Jupiter (Solana): токен по образцу tgm',
         dict(_pg, chain="solana", token_address=TOKEN_SOL)),
        # ── детализация позиции на Polymarket (нужна карточке DD и графу китов) ─────────
        # `market_id` НАРОЧНО ОСТАВЛЕН ПУСТЫМ: он подставляется живым id ПЕРЕД ОТПРАВКОЙ, а не
        # при сборке плана. Спроси мы скринер здесь - печать плана `--only next` сама сделала бы
        # сетевой вызов, то есть «ничего не отправлено» перестало бы быть правдой.
        ('prediction-market/position-detail', 'детализация позиции: адрес + ЖИВОЙ id рынка '
         '(подставится перед отправкой)', {"address": PM_WALLET, "market_id": LIVE_PM_MARKET}),
        # ── чей кошелёк и насколько он «умный» на перпах (нужно борду риска) ────────────
        ('tgm/position-intelligence', 'разведка по позициям токена',
         dict(_pg, token_symbol="BTC")),
        # ── №7 рейтинг коллеров против smart money: посты конкретного аккаунта ──────────
        ('ra-agent/posts-by-user', 'посты аккаунта: имя поля пользователя неизвестно',
         dict(_pg, username="nansen_ai")),
        # ── общий веб-поиск и рейтинг сетей: кандидаты для дайджеста ────────────────────
        ('search/web-search', 'веб-поиск: имя поля запроса неизвестно',
         dict(_pg, query="smart money")),
        ('chains/chain-rank', 'рейтинг сетей: тело пустое + пагинация', dict(_pg)),
        # ── DeFi-часть портфеля кошелька (нужна карточке DD) ────────────────────────────
        ('portfolio/defi-holdings', 'DeFi-холдинги кошелька', dict(_pg, address=WALLET)),
    ]
