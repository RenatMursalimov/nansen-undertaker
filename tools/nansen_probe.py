...
def _cases_ra():
    ...
    # ═══ РУЧКА ЖИВА: ПРОБА 22.09 ОТВЕТИЛА 422 С ИНСТРУКЦИЕЙ, А НЕ 404 ═══
    # Дословно: «Required field 'body -> date' is missing. Expected format:
    # {"from": "YYYY-MM-DD", "to": "YYYY-MM-DD"}». Это не приговор, а указание: путь
    # существует, тело разбирается, не хватает окна даты. Первая редакция этих кейсов окна не
    # посылала и поэтому проверила ровно одно - что путь есть. Второй заход обязан отвечать на
    # ГЛАВНЫЙ вопрос: приезжают ли посты и есть ли у них ЯЗЫК (без языка гибрид с китайским
    # срезом не собрать, и экран не нужен).
    #
    # ОКНО БЕРЁМ ТЕМ ЖЕ `_date_range`, ЧТО ОСТАЛЬНЫЕ ЭНДПОИНТЫ. Формат площадка назвала сама,
    # и он совпал с тем, что этот хелпер уже строит: своя копия формата даты разошлась бы с
    # общей на первой правке.
    return [
        ('ra-agent/posts-by-token', 'посты X по токену за 7 дней (мажор, ликвидный)',
         {"chain": "ethereum", "token_address": "0x1f9090aaE28b8a3dCeaDf281B0F12828e676c326",
          "date": N._date_range(7), "pagination": {"page": 1, "per_page": 10}}),
        ('ra-agent/posts-by-token', 'посты X по токену за 30 дней (шире окно, тот же токен)',
         {"chain": "ethereum", "token_address": "0x1f9090aaE28b8a3dCeaDf281B0F12828e676c326",
          "date": N._date_range(30), "pagination": {"page": 1, "per_page": 10}}),
        ('ra-agent/posts-by-token', 'посты X по токену (стейбл на base)',
         {"chain": "base", "token_address": _usdc_base,
          "date": N._date_range(7), "pagination": {"page": 1, "per_page": 10}}),
    ]
...
#: `ra` ремонт ВКЛЮЧАЕТ: схема этой ручки живым ключом не снята, и площадка сама называет,
#: чего не хватает (проба 22.09 вернула 422 с форматом окна даты). Ровно для такого случая
#: ремонт по словам площадки и написан - гадать тело руками, когда тебе его диктуют, незачем.
FIX_DEFAULT = ('new', 'pm', 'ra')
