...
def _price(v):
    ...
    if a >= 1:
        return '%.2f' % v
    # ── МЕЛКИЕ ЦЕНЫ: ЗНАЧАЩИЕ ЦИФРЫ, А НЕ ФИКСИРОВАННЫЕ ЧЕТЫРЕ ЗНАКА ──────────────────────
    # ЖИВОЕ ЗАМЕЧАНИЕ ВЛАДЕЛЬЦА: «в токенах где крупные единицы всё ок, а если PUMP - не
    # показывает после нуля единицы». Так и было: `%.4f` на цене $0.00348 даёт «0.0035», а на
    # $0.000012 - ровно «0.0000». Карта такого токена печатала «скопление между $0.00 и $0.00»,
    # то есть ГЛАВНАЯ строка экрана переставала быть диапазоном - тот же дефект, из-за которого
    # эта функция вообще появилась (там «между $62K и $62K»), просто с другого конца шкалы.
    # ЛЕЧИМ ТЕМ ЖЕ ПРАВИЛОМ: точность такая, при которой концы диапазона РАЗЛИЧАЮТСЯ. Для этого
    # нужны ЗНАЧАЩИЕ цифры: три штуки, где бы ни стоял нуль-разделитель. Хвостовые нули убираем,
    # иначе «0.00348000» читается как выдуманная точность.
    if a > 0:
        # %g даёт 3 значащие цифры, но на очень мелких переходит к «3.48e-06»: экспонента в
        # подписи цены - это уже не цена для человека, поэтому её разворачиваем обычной записью.
        _dig = max(2, 2 - int(math.floor(math.log10(a))) + 1)
        _t = ('%.*f' % (_dig, v)).rstrip('0').rstrip('.')
        return _t or '0'
    return '0'


def _axis_unit(hi):
    """ЕДИНИЦА ДЛЯ ВСЕЙ ОСИ ЦЕН, выбранная по её верхнему краю. -> (делитель, суффикс, знаки) | None.

    ЗАЧЕМ ЭТО ОТДЕЛЬНО ОТ `_price`. `_price` форматирует ОДНО число и переключает единицу на
    тысяче: $1.1K, но $796.21. На одной оси это даёт лестницу вида

        $5.2K · $4.5K · … · $1.1K · $796.21

    и читатель обязан в уме перевести последнюю строку в те же единицы, что остальные. Поймано на
    живом скриншоте владельца по ETH: ось из двадцати уровней, и ровно у нижнего единица другая.
    Числа при этом ВЕРНЫЕ - плохо именно то, что они несопоставимы глазом, а ось существует
    только ради сравнения.

    ПРАВИЛО: единица выбирается ОДИН раз по самому большому значению и применяется ко всем
    подписям оси. None означает «цены мелкие, единиц нет» - там работает `_price` со значащими
    цифрами (у мелких токенов свой разбор, см. его докстринг).
    """
    a = abs(hi or 0)
    if a >= 1e9:
        return (1e9, 'B', 2)
...
def _axis_price(v, unit):
    """Подпись цены В ЕДИНИЦЕ ОСИ. -> str."""
    if not unit:
        return _price(v)
    _d, _s, _p = unit
    return ('%.*f%s' % (_p, (v or 0) / _d, _s))


def _fmt_ladder(vals):
    """Форматы подписей цены ОТ ГРУБОГО К ТОЧНОМУ. -> [функция].

    Порядок не случаен: сначала единица оси (сопоставимо глазом), потом обычная запись `_price`,
    потом целые и дробные без единиц. Ниже по лестнице - точнее и длиннее.
    """
    _u = _axis_unit(max((abs(v or 0) for v in vals), default=0))
    out = []
    if _u:
        out.append(lambda v: _axis_price(v, _u))
    out.append(_price)
    for _p in (0, 1, 2, 4, 6):
        out.append(lambda v, _p=_p: '%.*f' % (_p, v or 0))
    return out


def _axis_labels(vals):
    """Подписи цен: ОДНА ЕДИНИЦА НА ВСЕХ И ВСЕ РАЗНЫЕ. -> [str].

    ДВА ПРАВИЛА СРАЗУ, И ВТОРОЕ СИЛЬНЕЕ ПЕРВОГО:
      1. одна единица на всю ось - иначе «$1.1K» и «$796.21» стоят рядом и несопоставимы глазом
         (поймано на живом скриншоте ETH);
      2. разные уровни обязаны иметь РАЗНЫЕ подписи - иначе ось перестаёт быть шкалой, а именно
         против этого `_price` и появилась («скопление между $62K и $62K»).
    Поэтому не фиксированный формат, а ПОИСК: берём первый по лестнице, при котором все подписи
    различны. Так ни одно из правил не нарушается молча, и правка формата не может тихо склеить
    два уровня - подбор сам уйдёт на шаг точнее.
    """
    vals = list(vals)
    for f in _fmt_ladder(vals):
...
def _price_pair(lo, hi):
    """Концы диапазона: одна единица, и они РАЗЛИЧАЮТСЯ. -> (str, str).

    Тот же поиск, что у оси, но на двух числах: «между $1.2K и $1.2K» - не диапазон, а вид
    диапазона. Живой случай: скопление между $1 150 и $1 190 - в тысячах оба конца округляются
    в «1.2K», и фраза перестаёт что-либо сообщать.
    """
    if lo == hi:
        _u = _axis_unit(abs(lo))
        return _axis_price(lo, _u), _axis_price(hi, _u)
    out = _axis_labels([lo, hi])
    return out[0], out[1]
    ...
    for i, r in enumerate(d['rows'], 1):
        ...
        L.append(('    on the map $%s across %d position(s) · longs $%s vs shorts $%s' if en
                 ...
                 % (_short(r['total']), int(r['shown'] or 0), _short(r['longs']),
                    _short(r['shorts'])))
        if r.get('named'):
            # ВСЯ КАРТА ИМЕНОВАНА - ГОВОРИМ СЛОВОМ, А НЕ ПОВТОРЯЕМ ТО ЖЕ ЧИСЛО.
            # ЖИВОЕ ЗАМЕЧАНИЕ ВЛАДЕЛЬЦА ПО БОРДУ: четыре строки подряд читались как
            # «на карте $944.1M … из этого $944.1M висит на кошельках с именем», то есть человек
            # сверяет глазами два одинаковых числа и ищет между ними разницу, которой нет.
            # В подписи КАРТЫ этот случай уже был вылечен месяц назад, а в борд правку не
            # перенесли - ровно тот класс расхождения, из-за которого числа считает один слой,
            # а формулировки приходится править в двух местах.
            if r['named'] >= (r.get('total') or 0) * 0.999:
                L.append(('    every position on this map sits on a wallet Nansen has a name for'
                          if en else
                          '    все позиции этой карты - на кошельках, которых Nansen знает '
                          'по имени'))
            else:
...
def liq_map_png(rows, token='', mark=None, lang='ru', out_dir=None, zoom=None):
    ...
    # ОДНА ЕДИНИЦА НА ВСЮ ОСЬ (см. `_axis_unit`): иначе лестница уровней смешивает «$1.1K» и
    # «$796.21», и нижнюю строку приходится переводить в голове.
    _labels = ['$%s' % t for t in _axis_labels([(b[0] + b[1]) / 2.0 for b in cl['buckets']])]
...
def liq_caption(cl, token='', lang='ru', on_image=False):
    ...
    # КОНЦЫ ДИАПАЗОНА - В ОДНОЙ ЕДИНИЦЕ, выбранной по большему из них: «между $796.21 и $5.2K»
    # заставляет сравнивать разные единицы внутри ОДНОЙ фразы.
    # НО ЕСЛИ В ЭТОЙ ЕДИНИЦЕ КОНЦЫ СОВПАЛИ - ВОЗВРАЩАЕМСЯ К ТОЧНОЙ ЗАПИСИ. Округление до 0.1K
    # склеивает $1 150 и $1 190 в «1.2K и 1.2K», то есть воспроизводит ровно тот дефект, из-за
    # которого `_price` вообще появилась («скопление между $62K и $62K»). Единая единица - правило
    # ради сопоставимости, различимость концов - правило ради смысла; второе сильнее.
    _lo_s, _hi_s = _price_pair(_lo, _hi)
