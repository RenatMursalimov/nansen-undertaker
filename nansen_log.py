...
SCENES = (
    ...
    # ═══ ДОЗОРНЫЙ (sentinel/). ДВЕ СЦЕНЫ, А НЕ ОДНА, И НЕ ОБЩАЯ СО СМАРТ-СДЕЛКАМИ: расход
    # дозорного идёт БЕЗ ЧЕЛОВЕКА, по факту движения рынка, и лёг он в общую сцену - вопрос
    # «сколько стоит наблюдение само по себе» стал бы невычислимым ровно там, где на него
    # смотрят (у автономной траты свой суточный потолок, см. sentinel/config.nansen_day_credits).
    'sentinel_watch',          # круг зажигания: лента smart-money/dex-trades живьём
    'sentinel_ignition',       # подтверждение ончейном движения с площадки (who-bought-sold)
)
...
# 'sentinel' СТОИТ ОТДЕЛЬНО ОТ 'cron' НАРОЧНО. Джоба по расписанию и живой дозорный тратят
# по-разному: первая - предсказуемо (столько-то раз в сутки), второй - по факту движения
# рынка, то есть в волатильный день втрое больше. Сложи их в одну поверхность, и вопрос
# «сколько стоит дозор» перестал бы иметь ответ.
SURFACES = ('chat', 'miniapp', 'cron', 'sentinel')
