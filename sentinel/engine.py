...
#: Разрешение горячего кольца. 60с, а не 15с: доходность за 15 минут от точности до секунды не
#: меняется, а памяти четыре точки на минуту стоят вчетверо. 90 минут глубины — ровно чтобы
#: закрыть 60-минутное окно с запасом на пропуски.
HOT_RES = 60
HOT_KEEP = 90 * 60
...
def _hot_put(x, ts):
    """Точка в горячее кольцо, если прошло разрешение. -> список точек инструмента."""
    arr = _HOT.setdefault(x.ticker, [])
    if arr and (ts - arr[-1][0]) < HOT_RES:
        # ПОСЛЕДНЮЮ ТОЧКУ ОБНОВЛЯЕМ, А НЕ ПРОПУСКАЕМ: свежайшая цена нужна для сравнения, и
        # «одна точка на минуту» не должна означать «цена минутной давности».
        arr[-1] = _row(x, ts)
        return arr
