...
def fix_polling(uid, now=None, lang=None):
    ...
    ПРОСЬБА ВЛАДЕЛЬЦА 26.09 дословно: «что если тогда в случае необходимости в Дозорном нужно
    кнопку перезагрузить его». Причина живая: юнит работал 7 часов со статусом `active`, кольцо
    было пустым, алертов не было полчаса - и починить это из чата было нечем.

    СЛУЖБЫ ИЗ ЧАТА НЕ РЕСТАРТУЕМ. `systemctl` по нажатию кнопки - это выполнение команд с
    правами root из мессенджера: одна такая дверь, и дальше её просят расширить («а ещё бота
    перезапустить», «а ещё pull сделать»). Граница здесь дешевле, чем потом.

    ЧТО ДЕЛАЕМ ВМЕСТО, И ПОЧЕМУ ЭТОГО ДОСТАТОЧНО: отдаём зависшую аренду. Опрос у нас берётся
    ПО АРЕНДЕ, и как только она свободна, её подхватывает живой процесс - бот умеет это сам
    (`engine._poller_dead`). То есть кнопка возвращает опрос, не трогая инфраструктуру.
    И ПЕЧАТАЕМ ЗАМЕРЫ, А НЕ «ГОТОВО»: сколько снимков за пять минут и за час, кто держал аренду
    и сколько секунд назад. «Починил» без чисел неотличим от «ничего не сделал».

    СЧЁТЧИК ДОСТАВОК НЕ ТРОГАЕМ, И ЭТО ВАЖНО СКАЗАТЬ ВЛАДЕЛЬЦУ: он спрашивал, не сбросит ли
    кнопка «147 из 25». Не сбросит - это ЖУРНАЛ ДОСТАВЛЕННОГО, а не счётчик состояния. Подделать
    его значило бы потерять ответ на «что мне присылали»; потолок поднимается кнопкой рядом.
    """
    import time as _tm
...
def esc_ui(s):
    """HTML-экранирование для экранов дозорного. Своя дверь не нужна - берём общую у карточек."""
...
_T = {
    ...
    'btn_fix': {'ru': '🩺 Проверить и починить опрос', 'en': '🩺 Check and fix polling'},
...
def menu_kb(uid, lang=None):
    ...
    rows = [
        ...
        # Дословно: «что если тогда в случае необходимости в Дозорном нужно кнопку перезагрузить
        # его». Причина просьбы живая: юнит `sentinel` работал 7 часов, служба показывала
        # `active`, а кольцо было пустым - и вылечить это из интерфейса было нечем.
        # ЧЕГО ЭТА КНОПКА НЕ ДЕЛАЕТ: не зовёт systemctl. У бота нет и не должно быть права
        # рестартовать службы - это дверь к выполнению команд с правами root по нажатию в чате.
        # ЧТО ОНА ДЕЛАЕТ ВМЕСТО: отдаёт зависшую аренду, после чего опрос подхватывает живой
        # процесс (бот умеет это сам, см. `engine._poller_dead`), и печатает ЗАМЕРЫ - сколько
        # снимков, кто держит аренду, сколько секунд назад. То есть решает ту же задачу, но
        # средствами, которые человеку не нужно давать второй раз.
        [B(_t('btn_fix', lang), callback_data='sen:fix')],
