...
#: СОЕДИНЕНИЕ НА ПОТОК. Не на процесс: ключ `onchain` НЕ в `db._SAME_THREAD_FALSE`, то есть
#: sqlite запрещает делиться соединением между потоками, а дозорный ходит в базу и из рабочих
#: потоков (`asyncio.to_thread`: `prune`, `digest_prune`, `outcome_due`, `ignition.scan`).
_TL = threading.local()


def conn():
    """Соединение дозорного. ОДНО НА ПОТОК, а не новое на каждый вызов.

    ═══ ЭТО НАСТОЯЩАЯ ПРИЧИНА `database is locked`, ПЕРЕЖИВШАЯ ДВА «ИСПРАВЛЕНИЯ» ═══
    ЖИВОЙ ПРОГОН ВЛАДЕЛЬЦА 26.09 НА ПРОДЕ (Python 3.12): `delivery_plan` падает
    `database is locked` при `in_transaction=False`, включённом WAL, ожидании 60 секунд, ОДНОМ
    потоке и tmp-файле, к которому никто больше не обращался. То есть пишущее соединение чистое,
    а лок держит КТО-ТО ЕЩЁ в этом же процессе.

    ЗАМЕР НАШЁЛ КОГО (`gc`-перепись объектов `sqlite3.Connection`): `db.get_conn` -> `_sqlite_conn`
    делает `sqlite3.connect(path)` НА КАЖДЫЙ ВЫЗОВ, кэша там нет вовсе. А `_one`/`_all` зовут
    `conn()` каждый раз. Одна серия чтений в `outbox.plan` (подписчики, настройки, виды, площадки,
    порог звонка, предохранитель, окно, пауза, потолок, счётчик) подняла число живых соединений с
    3 до 15. Каждое из них, пока живо, держит свой read-lock в WAL - и следующая ЗАПИСЬ упирается
    в чужой лок, а не в свой.

    ПОЧЕМУ ДВА ПРЕДЫДУЩИХ КРУГА ЛЕЧИЛИ НЕ ТО. Круг 2 закрывал курсоры, круг 7 добавил
    `rollback()` в `_end_read` - оба исправления верные и оба лечили ОДНО соединение, тогда как
    их плодилось по одному на вызов. Закрывать за собой транзакцию бесполезно, если следующее
    чтение открывает НОВОЕ соединение и оставляет его жить.

    ПОЧЕМУ ВЫЛЕЗЛО ИМЕННО СЕЙЧАС, А НЕ ДЕВЯТЬ КРУГОВ НАЗАД. Корень старый, но круги 8-9 добавили
    в путь доставки четыре новых чтения (`min_sev_for`, `burst_for`, `sent_in_window`,
    `nansen_cap`) - то есть почти удвоили число соединений на одну постановку в очередь. Плюс у
    владельца Python 3.12, где сборка мусора держит их дольше, чем 3.9 в разработке. Мой код не
    создал баг - он перевёл его из «иногда» в «всегда», и это тоже надо называть вслух.

    ПОЧЕМУ ПРАВИМ ЗДЕСЬ, А НЕ В `db.py`. Общую фабрику соединений используют все модули бота, и
    кэш там поменял бы поведение пятидесяти читателей сразу - включая те, что рассчитывают на
    своё отдельное соединение. Дозорный же уникален тем, что делает ДЕСЯТКИ чтений подряд перед
    записью, и именно ему нужна одна дверь. `db.ready` по-прежнему зовётся - он и создаёт таблицы
    (помнит это сам через `_ENSURED`).

    ═══ И ТОЛЬКО ПОД SQLITE. ПОД POSTGRES КЭШИРОВАТЬ НЕЛЬЗЯ ═══
    Под PG `db.get_conn` -> `_pg_conn` берёт соединение ИЗ ПУЛА (`pool.getconn`). Закэшировав
    его навсегда, мы бы НИКОГДА его не вернули - и пул исчерпался бы, остановив бота целиком.
    То есть лечение локальной блокировки sqlite убило бы прод, где база как раз PostgreSQL.
    А главное - там этой проблемы нет вовсе: пул и есть управляющий сроком жизни соединения,
    и «15 открытых дверей» под ним не возникает.
    ЭТА ВЕТКА ВАЖНЕЕ САМОГО ФИКСА: прод владельца на PG, тест на sqlite, и правка, проверенная
    только тестом, дошла бы до прода непроверенной стороной.
    """
    if getattr(db, 'DB_BACKEND', 'sqlite') == 'postgres':
        return db.ready('sentinel', _ensure, key=DB_KEY)
    c = getattr(_TL, 'conn', None)
    if c is not None:
        return c
    c = db.ready('sentinel', _ensure, key=DB_KEY)
    _TL.conn = c
    return c
...
# ══════════════════════════════════════════════════════════════════════════════════════════
def _one(sql, args=()):
    """Одна строка или None. Курсор закрывается, чтение ЗАВЕРШАЕТСЯ - всегда."""
    c = conn()
...
def _all(sql, args=()):
    """Все строки списком. Курсор закрывается, чтение ЗАВЕРШАЕТСЯ - всегда."""
    c = conn()
    cur = c.execute(sql, args)
    try:
...
def _end_read(c):
    """ЗАВЕРШИТЬ ЧИТАЮЩУЮ ТРАНЗАКЦИЮ. -> None.

    ═══ ЗАКРЫТЬ КУРСОР - НЕ ТО ЖЕ, ЧТО ЗАКРЫТЬ ТРАНЗАКЦИЮ ═══
    Прошлый круг закрывал курсоры и считал вопрос решённым. Живой прогон владельца показал, что
    нет: `database is locked` вернулся, и замеры назвали МЕСТО - отказ случается на первой
    ЗАПИСИ после серии ЧТЕНИЙ. В его выводе `event_new` (запись) прошёл, следующий
    `delivery_plan` (запись) упал, а между ними `outbox.plan` делает десяток SELECT: подписчики,
    настройки, виды, площадки, пауза, потолок, счётчик за сутки. Файл правильный, WAL включён,
    ожидание минута - значит это не нехватка терпения, а незавершённое чтение на соединении.
    ПОЧЕМУ НЕ ЛОВИЛОСЬ ЛОКАЛЬНО: момент, когда sqlite3 отпускает неявную транзакцию, зависит от
    версии Python (у владельца 3.12+, в разработке 3.9). «У меня зелено» тут не значит ничего, и
    правильный ответ - не подбирать версию, а НЕ ОСТАВЛЯТЬ за собой открытых чтений вовсе.
    БЕЗОПАСНО ЛИ ЭТО ДЛЯ ЗАПИСЕЙ: да, и это проверено по коду модуля - ни одна запись здесь не
    идёт «пачкой с чтением внутри». Каждая пишет и сразу коммитит (`snapshot_put` копит INSERT,
    но SELECT между ними не делает), поэтому отменять нечего. Если такой путь однажды появится,
    он обязан взять соединение сам и не ходить через `_one`/`_all` - правило записано здесь,
    чтобы нашлось раньше, чем баг.
    """
    try:
        if getattr(c, 'in_transaction', False):
            c.rollback()
    except Exception:
        try:
            c.rollback()
...
def diag():
    ...
    try:
        out.append('in_transaction=%s' % getattr(conn(), 'in_transaction', '?'))
    except Exception:
