...
| Sentinel: live alerts | ✅ Live on production | `sentinel/` (12 modules); five event kinds on Variational Omni plus Smart Ignition on Nansen; 154 offline checks in `tests/test_sentinel.py`; live proof in `proofs/sentinel_live_proof.py`; buttons in two menus; spec in [`SENTINEL_SPEC.md`](SENTINEL_SPEC.md), including the three defects live production found on day one |
