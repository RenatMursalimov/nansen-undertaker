...
| Sentinel: live alerts | ✅ Live on production | `sentinel/` (16 modules); ten event kinds across **three** venues (Variational Omni 553, Hyperliquid 234, Lighter 210 markets) plus Smart Ignition on Nansen; **a per-person flood fuse that presets cannot lift**, adjustable by button up to a hard ceiling, with everything past it leaving as one strength-ordered digest; settings re-checked at the Telegram door, so the off switch actually switches off; emergency stop `SENTINEL_DELIVER=0`; the funding unit **measured** by venue cross-check (`tools/sentinel_funding_unit.py` reproduces it without keys); address clusters counted as **people, not wallets**; a prediction-market bridge with three locks and **zero false matches** across 13 live instruments; 426 offline checks in `tests/test_sentinel.py`; live proof in `proofs/sentinel_live_proof.py`; buttons in two menus; spec in [`SENTINEL_SPEC.md`](SENTINEL_SPEC.md), including every defect live production found and the hypotheses it refuted |
