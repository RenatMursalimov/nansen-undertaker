...
| Sentinel: live alerts | ✅ Live on production | `sentinel/` (14 modules); ten event kinds across two venues (Variational Omni and Hyperliquid) plus Smart Ignition on Nansen; **a per-person flood fuse that presets cannot lift** (3 messages / 10 min) with everything past it leaving as one strength-ordered digest; settings re-checked at the Telegram door, so the off switch actually switches off; emergency stop `SENTINEL_DELIVER=0`; 348 offline checks in `tests/test_sentinel.py`; live proof in `proofs/sentinel_live_proof.py`; buttons in two menus; spec in [`SENTINEL_SPEC.md`](SENTINEL_SPEC.md), including every defect live production found and the hypotheses it refuted |
