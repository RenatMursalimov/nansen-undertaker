...
#: СИНТЕТИЧЕСКИЕ ЗАГЛУШКИ ИЗ ФИКСТУР И BASELINE. Это НЕ чьи-то кошельки: форма придумана
#: (повторяющийся нибл + счётчик), и по ней сразу видно, что адрес выдуман. Нужны потому, что
#: проверка «текст сцен не изменился» и режим репетиции обязаны работать на СТАБИЛЬНЫХ входах,
#: а стабильный вход - это константа в файле.
#:
#: ПОЧЕМУ СПИСКОМ, А НЕ ПРАВИЛОМ «повторяющиеся символы разрешены». Правило пришлось бы писать
#: шире, чем нужно, и однажды оно пропустило бы настоящий адрес, у которого случайно много
#: одинаковых символов. Список проверяем глазами один раз, правило - никогда.
ALLOWED_ADDR |= {
    ...
    '0x2222000000000000000000000000000000000022',
    '0x3333000000000000000000000000000000000033',
    # Входы трёх экранов, пришедших в мини-апп (DCA, держатели рынка с PnL). Та же форма
    # «видно, что выдумано», и добавлены они ПОСЛЕ того, как скруббер их поймал: список
    # проверяется глазами один раз, и именно поэтому он список, а не правило.
    '0x4444000000000000000000000000000000000044',
    '0x5555000000000000000000000000000000000055',
    '0x6666000000000000000000000000000000000066',
    '0x7777000000000000000000000000000000000077',
}
