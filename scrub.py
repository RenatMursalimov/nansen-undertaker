...
#: РУЧКИ АВТОРА ЗДЕСЬ НЕТ НАРОЧНО. Ссылка на конкурсное видео в выжимке есть, но она стоит АДРЕСОМ
#: (`https://x.com/...`), а не подписью «@…»: адрес разрешён точной строкой в сборщике, а класс
#: «@-ручка в тексте» остаётся запретным целиком. Разрешив ручку, мы ослепили бы проверку на целый
#: класс утечки ради одной подписи - и это уже было поймано негативным тестом.
#: Сравнение идёт в нижнем регистре (`m.group(0).lower()`), поэтому и список - в нижнем.
#: `@dataclass` попал сюда тем же правилом, что три декоратора рядом: это конструкция языка, а
#: не ручка в тексте. Поймано сборкой выжимки дозорного (`sentinel/variational_feed.py`), где
